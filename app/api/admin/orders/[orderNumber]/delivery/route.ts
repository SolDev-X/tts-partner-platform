...
export async function PATCH(request: Request, {params}: {params: Promise<{orderNumber: string}>}) {
  ...
  const order = await prisma.order.findUnique({where: {orderNumber}, select: {id: true, paymentStatus: true, deliveryStatus: true}});
  if (!order) return NextResponse.json({error: "Order not found"}, {status: 404});
  if (order.deliveryStatus === "CONFIRMED") return NextResponse.json({error: "客户已确认交付，无法继续修改。"}, {status: 409});

  const hasContent = Boolean(body.data.deliveryDescription || body.data.deliveryFileName || Object.values(body.data.deliveryData).some(Boolean));
